...
@test "Should show a global configuration and the current wd configuration" {
    ...
    assert_line -p -- "--config $HOME/.ngrok2/ngrok.yml"
    assert_line -p -- "--config $PWD/ngrok.yml"
}
...
@test "Should show configurations based on CONFIG_DIRS" {
    ...
    assert_line -p -- "--config $PWD/fixture/subcommand-configs/ngrok.yml"
}

@test "Should show configurations based on multiple CONFIG_DIRS" {
    CONFIG_DIRS="$PWD $PWD/fixture/subcommand-configs" run ngrok-autoload-config configs
    assert_line -p -- "--config $HOME/.ngrok2/ngrok.yml"
    assert_line -p -- "--config $PWD/ngrok.yml"
    assert_line -p -- "--config $PWD/fixture/subcommand-configs/ngrok.yml"
}
